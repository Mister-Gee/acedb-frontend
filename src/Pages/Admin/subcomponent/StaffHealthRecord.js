...
const StaffHealthRecord = ({offset, 
    perPage, 
    setPerPage, 
    setOffset, 
    setStaffHealthRecord, 
    slicedData, 
    searchData, 
    staffHealthRecordData, 
    contentLength,
    pageCount,
    handleEdit,
    handleDelete
    ...
    const handleSearch = (array, searchText) => {
        if (searchText === ''){
            setStaffHealthRecord(slicedData)
        }
        else{
            setStaffHealthRecord(search(array, searchText, 'fullName'))
        } 
    }
    return (
        <>
                ...
                <Col lg={12}>
                    <div className="table-header">
                        ...
                        <div className="search">
                            ...
                            <input type="search" placeholder='Name' className="search-box" id="search" onChange={(event) => handleSearch(searchData, event.target.value)}/>
                        </div>
                    </div>
                </Col>
            </Row>
                ...
                <Col lg={12}>
                    <div className="session-table">
                        <Table bordered hover responsive>
                            ...
                            <tbody>
                                {staffHealthRecordData.map((data, index) => (
                                    <tr key={data.id}>
