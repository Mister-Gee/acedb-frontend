...
import { examEndAttendance } from '../../../services/attendanceService';
...
const ExamEndAttendance = (props) => {
    ...
    const onSubmit = async(data) => {
        setIsSubmit(true)
        try {
            data.examEndingStudents = selectedStudent
            const res = await examEndAttendance(props.attId, data)
            if (res.status === 200 || res.status === 204){
                setIsSubmit(false)
                setAlertType("success")
                setMessage("Exam End Attendance Marked")
                setShowAlert(true)
...
export default ExamEndAttendance