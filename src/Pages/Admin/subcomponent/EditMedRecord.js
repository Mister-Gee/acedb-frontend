...
import {editAllRecord} from '../../../services/healthService';



const EditMedRecord = (props) => {
    ...
    const onSubmit = async(data) => {
        setIsSubmit(true)
        try {
            const res = await editAllRecord(props.medRecData.id, data)
            if (res.status === 200 || res.status === 204){
                setIsSubmit(false)
                setAlertType("success")
                setMessage("Medical Record Edited Successfully")
                setShowAlert(true)
                ...
                setIsSubmit(false)
                setAlertType("danger")
                setMessage("Failed to Edit Medical Record")
                setShowAlert(true)
            }
            setIsSubmit(false)
        }
    ...
    const initialValues = {
        bloodGroupID: props.medRecData.bloodGroupID,
        genotypeID: props.medRecData.genotypeID,
        weight: props.medRecData.weight,
        height: props.medRecData.height,
        familyDoctorName: props.medRecData.familyDoctorName,
        familyDoctorPhoneNumber: props.medRecData.familyDoctorPhoneNumber,
        additionalNote: props.medRecData.additionalNote,
        otherMedicalConditions: props.medRecData.otherMedicalConditions
    }
    ...
    return (
        ...
        <Modal.Header closeButton>
            <Modal.Title id="contained-modal-title-vcenter">
                Edit {props.medRecData.fullName} Medical Record
            </Modal.Title>
        </Modal.Header>
