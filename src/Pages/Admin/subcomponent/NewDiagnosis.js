...
import { addMedicalDiagnosis } from '../../../services/healthService';
...
const NewDiagnosis = (props) => {
    ...
    const onSubmit = async(data) => {
        setIsSubmit(true)
        try {
            const res = await addMedicalDiagnosis(data)
            console.log(res)
            if (res.status === 200 || res.status === 204){
                setIsSubmit(false)
                setAlertType("success")
                setMessage("Medical Diagnosis Added")
                setShowAlert(true)
                ...
                setIsSubmit(false)
                setAlertType("danger")
                setMessage("Failed to Add Medical Diagnosis")
                setShowAlert(true)
            }
        }
        catch(err){
