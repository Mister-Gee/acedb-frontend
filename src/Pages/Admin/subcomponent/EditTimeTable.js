...
const EditTimeTable = (props) => {
    ...
    const onSubmit = async(data) => {
        setIsSubmit(true)
        try {
            const res = await editTimetable(props.data.id, data)
            if (res.status === 200 || res.status === 204){
                setIsSubmit(false)
                setAlertType("success")
                setMessage("Exam Timetable Updated Successfully")
                setShowAlert(true)
                ...
                setIsSubmit(false)
                setAlertType("danger")
                setMessage("Failed to Update Exam Timetable")
                setShowAlert(true)
            }
            setIsSubmit(false)
        }
    ...
    const initialValues = {
        ...
        examDateTime: dateToInputDate(props.data.examDateTime),
        supervisorID: props.data.supervisorID,
        examStartTime: props.data.examStartTime,
        examDuration: props.data.examDuration,
                ...
                <form onSubmit={formik.handleSubmit} >
                    <Modal.Body className="">
                        ...
                        <Col lg={6} md={6} sm={12}>
                                    ...
                                    >
                                        <MenuItem value={props.data.courseID}>{props.data.course}</MenuItem>
                                    </StyledTextField>
                                </div>
                            </Col>
                            <Col lg={6} md={6} sm={12}>
                        ...
                        <Row>
                            ...
                            <Col lg={6} md={6} sm={12}>
                                    ...
                                    >
                                        <MenuItem value={props.data.supervisorID}>{props.data.supervisor}</MenuItem>
                                        {supervisor.map(data => (
